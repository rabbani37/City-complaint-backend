...
import { redisClient } from "./redist";

export const getBkashIdToken = async () => {
	try {
		const bkashIdTokenKey = `Bkash-service:idToken`;
		const bkashRefreshTokenKey = `Bkash-service:refreshToken`;
		...
		if (
			(bkashIdTokenTimes <= 60 * 10 || !bkashIdToken) &&
			bkashRefreshToken &&
		...
		const bkashApiResponse = await fetch(
		...
		throw new AppError(
			status.NOT_FOUND,
			`Failed to grant bKash token : ${error.message}`,
		);
	}
};
